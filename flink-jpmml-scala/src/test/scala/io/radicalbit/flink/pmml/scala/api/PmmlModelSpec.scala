...
class PmmlModelSpec extends WordSpec with Matchers with PmmlLoaderKit with PmmlEvaluatorKit {
  ...
  val derivableDense = DerivableVector[DenseVector]
  val derivableSparse = DerivableVector[SparseVector]

  "PmmlModel.validateInput" should {

    "return a Map[String, Any] from a trivial DenseVector" in {

      val inputVector = derivableDense.vector(DenseVector(1.0, 1.0, 1.0, 1.0))
      val result: Map[String, Any] = model.validateInput(inputVector)
      result shouldBe buildExpectedInputMap(DenseVector(1.0, 1.0, 1.0, 1.0),
                                            evaluator.model.getActiveFields.map(_.getName.getValue))
    }

    "return a Map[String,Any] from a non-trivial DenseVector" in {
      val inputVector = derivableDense.vector(DenseVector(2.0, -1.0, 3.0, 2.0))
      val result = model.validateInput(inputVector)
      result shouldBe buildExpectedInputMap(DenseVector(2.0, -1.0, 3.0, 2.0),
                                            evaluator.model.getActiveFields.map(_.getName.getValue))
    }

    "return a Map[String, Any] from a trivial SparseVector" in {
      val inputVector = derivableSparse.vector(SparseVector(4, Array(0, 1, 2, 3), Array(1.0, 2.0, 3.0, 4.0)))
      val result = model.validateInput(inputVector)
      result shouldBe buildExpectedInputMap(SparseVector(4, Array(0, 1, 2, 3), Array(1.0, 2.0, 3.0, 4.0)),
                                            evaluator.model.getActiveFields.map(_.getName.getValue))
    }

    "return a Map[String, Any] from a non-trivial SparseVector" in {
      val inputVector = derivableSparse.vector(SparseVector(4, Array(0, 2), Array(1.0, 2.0)))
      val result = model.validateInput(inputVector)
      result shouldBe buildExpectedInputMap(SparseVector(4, Array(0, 2), Array(1.0, 2.0)),
                                            evaluator.model.getActiveFields.map(_.getName.getValue))
    }
    ...
    "throw a InputValidationException if the dense input vector is not valid (exceed model dimension)" in {
      ...
      an[InputValidationException] should be thrownBy model.validateInput(derivableDense.vector(inputVector))
    }
    ...
    "throw a InputValidationException if the sparse input vector is not valid (exceed model dimension)" in {
      ...
      an[InputValidationException] should be thrownBy model.validateInput(derivableSparse.vector(inputVector))
    }
    ...
    "throw a EmptyEvaluatorException if the evaluator is an EmptyEvaluator" in {
      val inputVector = DenseVector(1.0, 3.0)
      an[EmptyEvaluatorException] should be thrownBy noneModel.validateInput(derivableDense.vector(inputVector))
    }
